...
const pool = mysql.createPool({
  ...
  password: process.env.DB_PASSWORD !== undefined ? process.env.DB_PASSWORD : '&rQ&fJT2',
...
module.exports = {
  ...
  query: async (text, params) => {
    ...
    // Switch from .execute() to .query() to use client-side interpolation.
    // .execute() uses server-side prepared statements which can fail on 
    // some DDL statements (like ALTER TABLE) in certain MariaDB versions.
    return promisePool.query(text, cleanParams);
  },
  pool
};
