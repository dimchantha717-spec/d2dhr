...
db.query('ALTER TABLE leave_requests ADD COLUMN IF NOT EXISTS approved_by VARCHAR(255), ADD COLUMN IF NOT EXISTS approved_at DATETIME')
    .then(() => console.log('✅ Auto-migrated leave_requests table for approved_by and approved_at.'))
    .catch(err => console.error('⚠️ Note: Auto-migration for approved_by failed or already applied:', err.message));

db.query('ALTER TABLE leave_requests ADD COLUMN IF NOT EXISTS resigned_reason TEXT DEFAULT NULL')
    .catch(() => {});
...
router.put('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const [result] = await db.query(
            ...
            [status, approvedBy || req.user.name || 'Admin', id]
        );
